...
| Email provider                                                                                                   | COMPLETE (Wave 7 branch, pending owner verification) | Resend REST (`lib/notifications/email.ts`); owner decision recorded 2026-09-08. Configure `RESEND_API_KEY` and `NOTIFICATIONS_FROM_EMAIL`; until a domain is verified, Resend only delivers to the account owner's address. |
